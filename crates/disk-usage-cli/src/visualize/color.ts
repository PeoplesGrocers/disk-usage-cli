...
let colorForFormats = (formats: FORMATS | 0
): Color => {
...
export let moduleTypeLabelInputPath = (inputPath: string, prefix: string): string => {
  ...
  if (color === esmColor) return prefix + 'Essential'
  if (color === cjsColor) return prefix + 'Ignored'
  return prefix + 'Essential & Ignored'
}
...
let formatLegendHTML = ''
  + `<span class="${styles.chit}" style="background:` + esmColor + '"></span>Essential<small>active work, programs, OS</small>'
  + `<span class="${styles.chit}" style="background:` + cjsColor + '"></span>Ignored File<small>build artifacts, generated</small>'
  + `<span class="${styles.chit}" style="background:` + otherColor + '"></span>Relocatable'
