...
let narrowSlice = (root: TreeNode, node: TreeNode, slice: Slice): void => {
  ...
  for (let child of parent.sortedChildren_) {
    if (child === node) {
      ...
      slice.sweepAngle_ =
        (child.bytesInOutput_ / totalBytes) * slice.sweepAngle_;
      break;
    }
...
export let createSunburst = (metafile: Metafile): HTMLDivElement => {
  ...
  timeline.stopClock("analyze sunburst");
  ...
  let startSunburst = (): [() => void, () => void] => {
    ...
    let drawNode = (
      ...
      if (flags & FLAGS.FILL) {
        ...
        if (
          hoveredNode &&
          (flags & FLAGS.HOVER || node.parent_ === hoveredNode)
        ) {
      ...
      // Handle the stroke
        ...
        let moveToRadius =
          flags & FLAGS.CHAIN || isFullCircle ? outerRadius : innerRadius;
        ...
        c.moveTo(
          centerX + moveToRadius * Math.cos(startAngle),
          centerY + moveToRadius * Math.sin(startAngle)
        );
        c.arc(
          centerX,
          centerY,
        ...
        if (!isFullCircle)
          c.lineTo(
            centerX + innerRadius * Math.cos(startAngle + clampedSweepAngle),
            centerY + innerRadius * Math.sin(startAngle + clampedSweepAngle)
          );
      }
      ...
      for (let child of node.sortedChildren_) {
        childTailEdge = drawNode(
          child,
          depth + 1,
          outerRadius,
          startAngle + (sweepAngle * bytesSoFar) / totalBytes,
          (child.bytesInOutput_ / totalBytes) * sweepAngle,
          childFlags,
          childTailEdge
        );
        bytesSoFar += child.bytesInOutput_;
    ...
    let draw = (): void => {
      ...
      drawNode(
        animatedNode,
        animatedDepth,
        computeRadius(animatedDepth),
        animatedStartAngle,
        animatedSweepAngle,
        FLAGS.ROOT | FLAGS.FILL,
        -Infinity
      );
    ...
    let tick = (): void => {
      ...
      animatedStartAngle =
        sourceStartAngle + (targetStartAngle - sourceStartAngle) * t;
      animatedSweepAngle =
        sourceSweepAngle + (targetSweepAngle - sourceSweepAngle) * t;
    ...
    let handleMouseMove = (e: MouseEvent): void => {
      ...
      if (node && node !== animatedNode.parent_) {
        ...
        if (node.parent_ && node.parent_.inputPath_ !== "") {
          ...
          tooltip =
            textToHTML(tooltip.slice(0, i)) +
            "<b>" +
            textToHTML(tooltip.slice(i)) +
            "</b>";
        } else {
          tooltip =
            "<b>" + textToHTML(shortenDataURLForDisplay(tooltip)) + "</b>";
        }
        if (colorMode === COLOR.FORMAT)
          tooltip += textToHTML(
  ...
  let startDetails = (): [() => void, () => void] => {
    ...
    let regenerate = (): void => {
      ...
      for (let child of children) {
        ...
        let nameEl = document.createElement("div");
        nameEl.className = styles.name;
        nameEl.innerHTML = textToHTML(
          name === child.inputPath_ ? shortenDataURLForDisplay(name) : name
        );
        rowEl.append(nameEl);
        ...
        barEl.className =
          styles.bar + (child.bytesInOutput_ ? "" : " " + styles.empty);
        barEl.style.background = bgColor;
        barEl.style.width =
          (100 * child.bytesInOutput_) / maxBytesInOutput + "%";
        sizeEl.append(barEl);
        ...
        bytesEl.textContent =
          colorMode === COLOR.FORMAT
            ? moduleTypeLabelInputPath(child.inputPath_, "")
            : size;
        ...
        if (node !== currentNode) {
          ...
          nodeEl.onclick = (e) => {
            ...
            if (lastInteractionWasKeyboard && generatedRows.length > 0) {
              ...
              generatedRows[
                !generatedNodes[0] && generatedRows.length > 1 ? 1 : 0
              ].focus();
            }
          };
        }
    ...
    return [
      ...
      () => {
        ...
        if (previousHoveredNode !== hoveredNode) {
          ...
          for (
            let node: TreeNode | null = hoveredNode;
            node;
            node = node.parent_
          ) {
  ...
  componentEl.innerHTML =
    ...
    "<p>" +
    "This visualization shows disk space usage across your files and directories. Click a segment to focus on it and expore its contents." +
    "</p>" +
    "<p>" +
