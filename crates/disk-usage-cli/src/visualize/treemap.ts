...
import { colorMode, timeline } from "./index";
import {
  COLOR,
...
let analyzeDirectoryTree = (metafile: Metafile): Tree => {
  ...
  for (let o in outputs) {
    ...
    let name = commonPrefix
      ? splitPathBySlash(o).slice(commonPrefix.length).join("/")
      : o;
    let node: TreeNodeInProgress = {
      name_: name,
      inputPath_: "",
      bytesInOutput_: 0,
      children_: {},
    ...
    for (let i in inputs) {
      let depth = accumulatePath(
        node,
        stripDisabledPathPrefix(i),
        inputs[i].bytesInOutput
      );
  ...
  let squarify = (
    ...
    while (start < sortedChildren.length) {
      ...
      for (let i = start; i < end; i++) {
        ...
        let upper = Math.round(
          (shortestSide * (areaInLayout + area)) / areaInRun
        );
        let [cx, cy, cw, ch] =
          w >= h
        ...
        children.push({
          ...
          box_: [cx, cy, cw, ch],
          children_:
            cw > CONSTANTS.INSET_X && ch > CONSTANTS.INSET_Y
              ? layoutTreemap(
...
export let createTreemap = (metafile: Metafile): HTMLDivElement => {
  let HACK_TREE = analyzeDirectoryTree(metafile);
  timeline.stopClock("analyze treemap");
  ...
  let bgColor = "";
  let fgOnColor = "";
  let normalFont = "14px sans-serif",
    boldWidthCache: Record<number, number> = {};
  let boldFont = "bold " + normalFont,
  ...
  let resize = (): void => {
    ...
    if (width !== oldWidth || height !== oldHeight) {
      layoutNodes = layoutTreemap(
        HACK_TREE.root_.sortedChildren_,
        0,
        0,
        width - 1,
        height - 1
  ...
  let tick = (): void => {
    ...
    if (
      animationBlend !== oldAnimationBlend ||
  ...
  let drawNodeBackground = (
    ...
    if (culling !== Culling.Culled && !node.isOutputFile_) {
      c.fillStyle = canvasFillStyleForInputPath(
        c,
        node.inputPath_,
        bgOriginX,
        bgOriginY,
  ...
  let drawNodeForeground = (
    ...
    if (h >= CONSTANTS.HEADER_HEIGHT) {
      ...
      if (nameText === node.name_ && node.sortedChildren_.length) {
        let detailText =
          " – " +
          (colorMode === COLOR.FORMAT
            ? moduleTypeLabelInputPath(node.inputPath_, "")
            : node.sizeText_);
        let [sizeText, sizeWidth] = textOverflowEllipsis(
          detailText,
          maxWidth - nameWidth
        );
      ...
      if (h > CONSTANTS.INSET_Y + 16 && !node.sortedChildren_.length) {
        ...
        c.fillText(
          sizeText,
          x + Math.round((w - sizeWidth) / 2),
          y + CONSTANTS.HEADER_HEIGHT + Math.round(h - CONSTANTS.INSET_Y) / 2
        );
        c.globalAlpha = 1;
      }
  ...
  let draw = (): void => {
    ...
    for (let node of layoutNodes) {
      ...
      if (
        currentLayout ||
        (nodeContainingHover && node !== nodeContainingHover)
      ) {
        ...
        c.globalAlpha =
          ...
          (!currentLayout ||
          (!animationSource &&
            nodeContainingTarget &&
            node !== nodeContainingTarget)
            ? 1
            : transition);
        c.fillStyle = bgColor;
        c.fillRect(x, y, w, h);
  ...
  let showTooltip = (x: number, y: number, html: string): void => {
    ...
    for (
      let el: HTMLElement | null = tooltipEl;
      el;
  ...
  let hitTestNode = (
    mouseEvent: MouseEvent | WheelEvent
  ): NodeLayout | null => {
    let visit = (
      nodes: NodeLayout[],
      isTopLevel: boolean
    ...
    return currentLayout
      ? visit([currentLayout], false)
      : visit(layoutNodes, true);
  };
  ...
  let updateHover = (e: MouseEvent): void => {
    ...
    if (layout) {
      let node = layout.node_;
      let tooltip =
        node.name_ === node.inputPath_
          ? shortenDataURLForDisplay(node.inputPath_)
          : node.inputPath_;
      let nameSplit = tooltip.length - node.name_.length;
      tooltip =
        textToHTML(tooltip.slice(0, nameSplit)) +
        "<b>" +
        textToHTML(tooltip.slice(nameSplit)) +
        "</b>";
      tooltip +=
        colorMode === COLOR.FORMAT
  ...
  let changeHoveredNode = (node: TreeNode | null): void => {
    if (hoveredNode !== node) {
      hoveredNode = node;
      canvas.style.cursor =
        node && !node.sortedChildren_.length ? "pointer" : "auto";
      invalidate();
    }
  };
  ...
  let searchFor = (
    ...
    for (let child of children) {
      let result =
        child.node_ === node ? child : searchFor(child.children_, node);
  ...
  componentEl.innerHTML =
    ...
    "<p>" +
    "This visualization shows disk space usage across your files and directories. " +
